...
| `chain_name`       | `string`             | Yes           | Unique, lowercased name of the chain, often used for folder names and identification in registries.                                                                                                                                                                      |
...
| `bech32_prefix`    | `string`             | Yes (Cosmos)  | The Bech32 address prefix (used in Cosmos-based addresses).                                                                                                                                                                                                              |
...
| `slip44`           | `number`             | Optional      | The SLIP-0044 coin type used for generating HD wallet paths (commonly `118` for Cosmos chains).                                                                                                                                                                          |
...
| `metadata.op_bridge_id` | `string`        | Optional      | The OP bridge identifier, if the chain is connected via OP bridging.                                                                                                                                                                                                      |
| `metadata.op_denoms`    | `string[]`      | Optional      | One or more denoms used in OP bridging.                                                                                                                                                                                                                                  |
| `metadata.executor_uri` | `string`        | Optional      | Executor service endpoint for advanced chain operations.                                                                                                                                                                                                                 |
| `metadata.ibc_channels` | `object[]`      | Optional      | An array describing IBC channel details (e.g., `channel_id`, `port_id`, `version`, and the counterpart `chain_id`).                                                                                                                                                      |
| `metadata.assetlist`    | `string (URL)`  | Optional      | URL pointing to the `assetlist.json` file for this chain.                                                                                                                                                                                                                |
| `metadata.minitia`      | `object`        | Optional      | Information specific to the chain’s rollup type, with fields like `type` (e.g. `minievm`, `minimove`) and `version`.                                                                                                                                                     |
